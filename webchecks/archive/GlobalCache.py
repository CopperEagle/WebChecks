...
class GlobalCache(object):
    ...
    def get_link_location(self, weblink : str) -> str:
        """Get the local location where the content for a given URL is stored. None if it is not stored.
    ...
    def store(self, domain : str, content : Union[str, bytes], name : str, sec_before_refresh : Union[int, float]):
        """
        ...
        cu.execute("INSERT INTO metadata VALUES (?, ?, ?, ?)", 
            (domain, name, hash, sec_before_refresh))
        self.db.commit()
        return hash

    def load(self, domain : str, name : str, is_binary : bool = False) -> Union[None, str, bytes]:
        """
        Load the file. None if not found or expired.
        ...
        """
        cu = self.db.cursor()
        for sbr in cu.execute("SELECT sec_before_refresh FROM metadata WHERE name = ? AND domain = ?", (name, domain)):
            if time.time() > sbr[0]:
                cu.execute("DELETE FROM metadata WHERE name = ? AND domain = ?", (name, domain))
                self.db.commit()
                return (None, None)
    ...
    def get_hash(self, domain : str, name : str) -> Union[None, str]:
        ...
        """
        cu = self.db.cursor()
        for sbr, hash in cu.execute("SELECT sec_before_refresh, hash FROM metadata WHERE name = ? AND domain = ?", (name, domain)):
            if time.time() > sbr:
                cu.execute("DELETE FROM metadata WHERE name = ? AND domain = ?", (name, domain))
                self.db.commit()
                return None

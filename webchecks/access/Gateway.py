...
from webchecks.utils.url import extract_fully_qualified_domain_name, change_protocol, is_url, extract_protocol
...
from webchecks.config import config, ENABLE_JAVASCRIPT, LOG_INFO, LOG_ERROR, LOG_DEBUG, ENFORCE_HTTPS
...
class GateWay(object):
	...
	At the present time it does not allow any other protocol other than https.
	If there is a link that has a protocol other than https, say http, it will
	be changed to https.
	"""
	...
	def _request_resource(self, linkpair : str) -> Tuple[bytes, dict, str]:
		link = linkpair.url
		log_link(link)
	...
	def express_request(self, link : str) -> Tuple[bytes, dict, str]:
		...
		for req in ret:
			if "robots.txt" in req[2]:
				return req
		logging(f"Express request failed, probably due to opaque redirect.", LOG_ERROR)
		return (b"", {}, original_url)

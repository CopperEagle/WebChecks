...
from webchecks.config import config, RESULT_STORAGE_LOCATION, LOG_INFO, LOG_ERROR, DO_CRAWL


class AccessHead(object):
	"""Accesshead maintains the run and initiates accesses."""
	def __init__(self, initial_seed_urls):
		self.gateway = None
		self.initial_seed_urls = \
			[initial_seed_urls] if type(initial_seed_urls) == str else initial_seed_urls
	...
	def run(self, gateway : GateWay, max_time_s : Union[int, float] = 1000):
		...
		for url in self.initial_seed_urls:
			if not gateway.add_to_queue(url):
				raise ValueError(f"REJECTED initial seed {url}. Conflicting security policy likely \
the case. See log for more info. If you need more information, you may change LOGGING_LEVEL.")
		...
		while True:
			...
			if gateway.done():
				logging(f"Done: No more links to process. Early terminating.", LOG_INFO)
				break
			
			if time.time() - start_timestamp >= max_time_s:
				logging(f"Maximum time reached. Aborting the process.", LOG_INFO)
				break
			time.sleep(0.1)
